// rA
// Fixed /spec bug with steam users
// Prevent glow from absolute invisible
...
public plugin_init()
{
	...
	register_concmd("amx_glow", "cmd_glow", ADMIN_LEVEL_D, "<nick, #userid, authid, or @team/@all> <color> (or) <rrr> <ggg> <bbb> -- lasts 1 round")
	register_concmd("amx_glow2", "cmd_glow", ADMIN_LEVEL_D, "<nick, #userid, authid, or @team/@all> <color> (or)  <rrr> <ggg> <bbb> -- lasts forever")
...
public cmd_glow(id, level, cid)
{
	...
	new command[16], arg1[32], arg2[32], arg3[32], arg4[32]
	...
	new rnum, gnum, bnum, bool:isOff = false;
	if(valid)
	{
	...
	else if(!valid)
	{
		...
		if(rnum == 0 && gnum == 0 && bnum == 0) isOff = true;
	}
	else
	{
	...
	if(arg1[0] == '@')
	{
		...
		for( new i = 0; i < pnum; i++ )
		{
			...
			fm_set_rendering(pid, kRenderFxGlowShell, rnum, gnum, bnum, kRenderTransAlpha, 255)
		}
		switch(activity)
		{
...
public sp_off(id) 
{
	...
	{
		fm_set_rendering( id, kRenderFxGlowShell, g_glow[id][0], g_glow[id][1], g_glow[id][2], kRenderTransAlpha, 255)
	}
