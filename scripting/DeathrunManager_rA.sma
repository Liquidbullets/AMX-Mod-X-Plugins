...
public GetIdentifier(id,szIdentifier[]) {
	get_user_authid(id,szIdentifier,31);

	if(!IsValidSteamID(szIdentifier)) {
		get_user_ip(id,szIdentifier,31,1);
	}
}
...
public client_authorized( id ) {
	...
	if(iResultCode) {
		if((iTimestamp+g_pBanTime)<iCurrentTime) {
			...
			server_cmd("kick #%d Temporary banned for quiting as Terrorist.",get_user_userid(id));
		}
	} else {
		g_bConnected[ id ] = true;
	}
