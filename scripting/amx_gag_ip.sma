/*
	Gag plugin made by xPaw and Exolent, edited by Firippu for gag menu IP support.
*/
...
public client_authorized( id )
{
	...
	if( !IsValidSteamID( g_szSteamID[ id ] ) )
	{
		get_user_ip( id, g_szSteamID[ id ], 34, 1 )
	}
}
...
public FwdThink( const iEntity )
{
	...
	for( new i = 0; i < g_iGagged; i++ )
	{
		...
		{
			if( IsValidSteamID( data[ GAG_STEAMID ] ) )
			{
				id = find_player( "c", data[ GAG_STEAMID ] );
			}
			else
			{
				id = find_player( "d", data[ GAG_STEAMID ] );
			}
	...
	else
	{
		if( iNumRemoveSteamIDs )
		{
			...
			for( new i = 0; i < iNumRemoveSteamIDs; i++ )
			{
				...
				iLen += formatex( szQuery[ iLen ], charsmax( szQuery ) - iLen, "%splayer_steamid = ^"%s^"", i ? " OR " : "", data[ GAG_STEAMID ] );
			}
...
DeleteAllGags( )
{
	...
	for( new i = 0; i < g_iGagged; i++ )
	{
		...
		{
			iPlayer = find_player( "d", data[ GAG_STEAMID ] );
		}
